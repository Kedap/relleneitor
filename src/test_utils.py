...
def generate_testing_schemas() -> Dict[str, Tuple[Table, int]]:
    ...
    usuarios_table = Table(
        ...
        columns=[
            ...
            Column(
                name="fecha_registro", type="DATE", faker_provider="date_this_decade"
            ),
    ...
    articulos_table = Table(
        ...
        columns=[
            ...
            Column(
                ...
                type="DATETIME",
                faker_provider="date_time_this_month",
            ),
            Column(
                name="categoria_id",
                type="INTEGER",
    ...
    comentarios_table = Table(
        ...
        columns=[
            ...
            Column(
                name="fecha", type="DATETIME", faker_provider="date_time_this_month"
            ),
        ],
    )
...
def generate_testing_sql(
    output_dir: str = "testing", prefix: str = "test_data"
) -> Tuple[int, int]:
    """
...
def generate_custom_testing_sql(
    tables_config: List[Tuple[str, List[Tuple[str, str, Optional[str]]], int]],
    ...
    for table_name, columns_data, num_rows in tables_config:
        ...
        for col_name, col_type, faker_provider in columns_data:
            columns.append(
                Column(name=col_name, type=col_type, faker_provider=faker_provider)
            )
    ...
    productos_table = Table(
        ...
        columns=[
            ...
            Column(
                name="precio",
                type="DECIMAL",
                faker_provider="random_int(min=100,max=10000)",
            ),
            Column(
                name="categoria_id",
                type="INTEGER",
            ...
            Column(
                name="stock", type="INTEGER", faker_provider="random_int(min=0,max=100)"
            ),
            Column(
                name="codigo",
                type="VARCHAR(20)",
                faker_provider="bothify(text='??-####-????')",
            ),
        ],
        primary_key="id",
    )
    ...
    pedidos_table = Table(
        ...
        columns=[
            ...
            Column(
                name="total",
                type="DECIMAL",
                faker_provider="random_int(min=1000,max=100000)",
            ),
            Column(
                name="estado",
                type="VARCHAR(20)",
                faker_provider="random_element(elements=('Pendiente', 'Pagado', 'Enviado', 'Entregado', 'Cancelado'))",
            ),
        ],
        primary_key="id",
    )
    ...
    detalles_pedido_table = Table(
        ...
        columns=[
            ...
            Column(
                name="cantidad",
                type="INTEGER",
                faker_provider="random_int(min=1,max=10)",
            ),
            Column(
            ...
            Column(
                name="subtotal",
                type="DECIMAL",
                faker_provider="random_int(min=100,max=100000)",
            ),
        ],
        primary_key="id",
    )
